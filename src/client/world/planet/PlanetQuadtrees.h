...
public:
     ...
     */
    void debug_draw(const glm::vec3& originRender, const PlanetLodParams& params, DebugMode mode = DebugMode::Level,
                    int segmentsPerEdge = 6) const;
...
private:
    ...
    void debug_draw_node(uint32_t index, const glm::vec3& originRender, const PlanetLodParams& params, DebugMode mode,
                         int segmentsPerEdge) const;

    /// Corner of a node on the sphere. cu,cv in [0,1] inside the node
    [[nodiscard]] glm::dvec3 node_point(uint32_t index, double cu, double cv, const PlanetLodParams& params) const;
