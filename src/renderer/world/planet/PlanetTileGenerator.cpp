...
namespace vp {
    PlanetTileGenerator::PlanetTileGenerator(PlanetTerrainParams params, uint16_t resolution,
                                             double planetRadius, unsigned workerCount)
    ...
    void PlanetTileGenerator::submit_pending(uint32_t maxSubmit) {
        ...
        if (m_keyScratch.size() < count) m_keyScratch.resize(count);
    ...
    void PlanetTileGenerator::generate(const PlanetTileKey &key, PlanetTileData &out,
        ...
        for (int y = 0; y < res; ++y) {
            for (int x = 0; x < res; ++x) {
                const glm::dvec3 dir = face_uv_to_direction(
                    key.face(), u0 + static_cast<double>(x) * step, v0 + static_cast<double>(y) * step);
        ...
        sampler.sample_height_batch(dirX.data(), dirY.data(), dirZ.data(), count,
                                    key.level(), heights.data(), scratch);
