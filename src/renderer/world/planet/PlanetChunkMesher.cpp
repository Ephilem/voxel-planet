...
PlanetChunkMesher::MesherTaskOutput PlanetChunkMesher::build_mesh(const MesherTaskInput &input) {
        ...
        for (int y = 0; y < CHUNK_SIZE; ++y) {
            for (int z = 0; z < CHUNK_SIZE; ++z) {
                ...
                for (int faceDir = 0; faceDir < 6; faceDir++) {
                    ...
                    face.z = static_cast<uint32_t>(z * 16);
    ...
    //                     const uint8_t nbH = static_cast<uint8_t>(nb >> 8);
    //                     if (nbTexID == 0 || blkH > nbH)
    //                         allMasks[2][y][static_cast<uint32_t>(voxel) | (static_cast<uint32_t>(nbH) << 16)][x] |= (
    //                             1u << z);
    //                 }
    //                 // Face 3: +Y, check y+1 (side face)
    //                 {
    ...
    //                     const uint8_t nbH = static_cast<uint8_t>(nb >> 8);
    //                     if (nbTexID == 0 || blkH > nbH)
    //                         allMasks[3][y][static_cast<uint32_t>(voxel) | (static_cast<uint32_t>(nbH) << 16)][x] |= (
    //                             1u << z);
    //                 }
    //                 // Face 4: -Z (toward planet center), check z-1
    ...
    //                     const uint8_t nbH = static_cast<uint8_t>(nb >> 8);
    //                     if (nbTexID == 0 || nbH < 15)
    //                         allMasks[4][z][static_cast<uint32_t>(voxel)][y] |= (1u << x);
    //                 }
    //                 // Face 5: +Z (away from planet, top/grass face), check z+1
    ...
    //                         allMasks[5][z][static_cast<uint32_t>(voxel)][y] |= (1u << x);
    //                 }
    //             }
    //         }
    //     }
    // }
    //
    // // Z is the altitude axis (radial, away from planet center).
    // // Faces 4/5 (±Z) are top/bottom (toward/away from planet).
    // // Faces 0/1/2/3 (±X, ±Y) are side faces.
    // // {slice_axis, u_axis, v_axis}
    // constexpr int FACE_AXES[6][3] = {
    //     {0, 1, 2}, {0, 1, 2}, // ±X: slice=X, u=Y, v=Z
    //     {1, 0, 2}, {1, 0, 2}, // ±Y: slice=Y, u=X, v=Z
    //     {2, 0, 1}, {2, 0, 1}, // ±Z: slice=Z, u=X, v=Y
    // }; {
    ...
    //
    //         // Side faces have Z as their v-axis. Partial blocks must NOT merge
    //         // along Z (altitude): there are gaps between their tops.
    //         const bool isSideFace = (faceDir <= 3);
    //
    ...
    //                         face.x = p[0];
    //                         face.y = p[1];
    //                         // z is stored in sub-voxel units (value / 16.0 = voxel altitude).
    //                         // +Z face (faceDir==5) encodes exact top position; side faces encode base + nbH offset.
    //                         face.z = (faceDir == 5)
    //                                      ? static_cast<uint32_t>(p[2] * 16 + blkH)
    //                                      : (isSideFace
    //                                             ? static_cast<uint32_t>(p[2] * 16 + nbH)
    //                                             : static_cast<uint32_t>(p[2] * 16));
    //                         face.faceIndex = faceDir;
    //
    //                         // Width/height in sub-voxel units (stored as value - 1).
    //                         // The Z-direction of a side face uses partial-block height when applicable.
