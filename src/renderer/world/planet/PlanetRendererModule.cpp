...
void PlanetRendererModule::init_renderers(flecs::world &ecs) {
    ...
    m_tileAtlas = std::make_unique<PlanetTileAtlas>(
    ...
    ecs.set<PlanetTileAtlasRef>({
        .atlas = m_tileAtlas.get(),
        .renderer = m_tileRenderer.get()
    });
}
...
void PlanetRendererModule::register_systems(flecs::world &ecs) {
    ecs.observer<const PlanetTileLodComp, const PlanetTerrainParams, const PlanetComp>("PlanetRendererModule-SetupStreamer")
        .event(flecs::OnAdd)
        .each([](flecs::entity e, const PlanetTileLodComp &lod, const PlanetTerrainParams &terrainParams, const PlanetComp &comp) {
            auto* streamer = e.get<PlanetTileStreamComp>();
            if (!streamer) {
                auto generator = std::make_unique<PlanetTileGenerator>(terrainParams, PLANET_TILE_ATLAS_RESOLUTION, comp.radius);
