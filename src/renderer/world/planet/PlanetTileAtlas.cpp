...
PlanetTileAtlasKey PlanetTileAtlas::acquire_slot() {
    ...
    for (uint32_t i = 0; i < m_capacity; ++i) {
        ...
        if (slot.key.level() <= PLANET_TILE_ATLAS_PINNED_LEVEL)
            continue;

        if (slot.lastUsedFrame >= m_currentFrame)
            continue;
    ...
    if (victim == m_capacity)
        return INVALID_ATLAS_SLOT; // full
...
PlanetTileAtlasKey PlanetTileAtlas::upload(nvrhi::ICommandList* cmd, const PlanetTileKey& key,
    ...
    if (data.resolution != PLANET_TILE_ATLAS_RESOLUTION) {
        LOG_ERROR("PlanetTileAtlas", "Tile resolution {} does not match atlas resolution {}", data.resolution,
                  PLANET_TILE_ATLAS_RESOLUTION);
        ++m_stats.failedUploads;
        return INVALID_ATLAS_SLOT;
    }
    ...
    if (data.heightmap.size() != expected) {
        LOG_ERROR("PlanetTileAtlas", "Tile heightmap has {} texels, expected {}", data.heightmap.size(), expected);
        ++m_stats.failedUploads;
        return INVALID_ATLAS_SLOT;
    }
    ...
    if (const auto it = m_lookup.find(key); it != m_lookup.end()) {
        ...
        if (slot == INVALID_ATLAS_SLOT) {
            LOG_WARN("PlanetTileAtlas", "Atlas full ({} slots all used this frame), tile dropped", m_capacity);
            ++m_stats.failedUploads;
            return INVALID_ATLAS_SLOT;
        }
    ...
    const auto subresource =
        nvrhi::TextureSubresourceSet().setBaseMipLevel(0).setNumMipLevels(1).setBaseArraySlice(slot).setNumArraySlices(
            1);
...
void PlanetTileAtlas::init_gpu() {
    ...
    if (m_capacity < requestedCapacity) {
        LOG_WARN("PlanetTileAtlas", "Device limits the atlas to {} slices (requested {})", m_capacity,
                 requestedCapacity);
    }
    ...
    LOG_INFO("PlanetTileAtlas", "Allocated {} slices of {}x{} R16_UNORM ({} MiB)", m_capacity,
             PLANET_TILE_ATLAS_RESOLUTION, PLANET_TILE_ATLAS_RESOLUTION,
             (size_t(m_capacity) * PLANET_TILE_ATLAS_RESOLUTION * PLANET_TILE_ATLAS_RESOLUTION * sizeof(uint16_t)) /
                 (1024 * 1024));
}
} // namespace vp
