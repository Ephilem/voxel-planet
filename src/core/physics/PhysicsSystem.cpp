    ...
    ecs.system<Velocity, Transform, RigidBody>("Physics-Collision")
        ...
        .each([](flecs::entity e, Velocity& vel, Transform& transform, RigidBody& body) {
            ...
            for (int axis = 0; axis < 3; axis++) {
                ...
                // Check all block on the final AABB, if any is solid, move back to the edge of the block and stop
                // movement on this axis.
                ...
                // TODO Other collision checks (non-axis-aligned) will be handled after the full movement is applied, so
                // we can react to the final position.
            }
                ...
                if (stepY > 0.0f && stepY <= kStepHeight) {
                    ...
                    if (aabbClear(steppedPos)) {
                        ...
                        if (blockedX) {
                            ...
                            if (aabbClear(testPos))
                                steppedPos.x = testPos.x;
                            else
                                canStep = false;
                        }
                        ...
                        if (canStep) {
                            ...
                            vel.y = 0.0f;
                            if (blockedX)
                                vel.x = originalDeltaX / dt;
                            if (blockedZ)
                                vel.z = originalDeltaZ / dt;
                        }
                    }
                }
            }
            ...
            body.onGround = false;
            {
                ...
                AABB aabb = AABB{glm::vec3(pos.x - halfExt.x, probeY, pos.z - halfExt.z),
