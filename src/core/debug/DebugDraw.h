...
void Circle(const glm::vec3& center, float radius, const glm::vec4& color, int segments = 16,
            const glm::vec3& normal = {0, 1, 0});

inline void Aabb(const AABB& aabb, const glm::vec4& color) {
    Aabb(aabb.min, aabb.max, color);
}
