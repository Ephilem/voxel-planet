...
namespace vp {
    ...
    inline glm::dvec3 face_uv_to_direction(CubemapFace face, double u, double v) {
        ...
        glm::dvec3 dir;
        switch (face) {
            case FACE_POS_X: dir = {1.0, b, -a}; break;
            case FACE_NEG_X: dir = {-1.0, b, a}; break;
            case FACE_POS_Y: dir = {a, 1.0, -b}; break;
            case FACE_NEG_Y: dir = {a, -1.0, b}; break;
            case FACE_POS_Z: dir = {a, b, 1.0}; break;
            case FACE_NEG_Z: dir = {-a, b, -1.0}; break;
     ...
     */
    inline void direction_to_face_uv(const glm::dvec3 &dir, CubemapFace &out_face,
    ...
    inline double planet_radius_for_voxel_size(double voxelSize, uint8_t maxLevel,
                                               uint32_t chunkSize = CHUNK_SIZE) {
        return voxelSize * double(chunkSize) * double(1u << maxLevel)
               / 1.5707963267948966192; // / (pi/2)
    }

    inline double planet_snap_radius(double radius, uint8_t maxLevel,
                                     uint32_t chunkSize = CHUNK_SIZE) {
     ...
     */
    inline bool planet_is_voxel_aligned(double radius, uint8_t maxLevel,
                                        uint32_t chunkSize = CHUNK_SIZE) {
