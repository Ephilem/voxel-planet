...
inline flecs::entity get_first_ancestor_grid(flecs::entity e) {
    ...
    LOG_FATAL("Spatial",
              "A entity with a CellCoord or other spatial specifics component MUST be child of a grid. No ancestor "
              "with Grid component found for entity {}",
              e.id());
    std::abort();
}
 ...
 * Will get the parent, the grid and his CellCoord and Transform to calculate the HP position of the entity in the grid
 * space
 * @param entity the entity. It should be a child of a grid, and have a CellCoord and Transform component.
 * @return return calculated position. If it doesnt have a parent with a Grid component, return only the transform
...
inline glm::dvec3 get_hp_position(flecs::entity entity) {
    ...
    if (!parent.is_valid())
        return t ? glm::dvec3(t->pos) : glm::dvec3(0.0);
