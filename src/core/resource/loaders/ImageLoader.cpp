...
std::shared_ptr<ImageResource> ImageLoader::load_typed(const std::string& name) {
    ...
    if (fail_reason) {
        ...
        throw std::runtime_error("Image resource loader failed to load file '" + fullFilePath +
                                 "' with error: " + std::string(fail_reason));
    }
    ...
    size_t data_size =
        static_cast<size_t>(width) * static_cast<size_t>(height) * static_cast<size_t>(requiredChannelCount);
